...
**Inputs read**: WORKLOG.md (TASK-002, P5 close), all warmup/base_method/smc __init__.py and ENTRY files, P5.12 VI kickoff table
**Scope**: 24 base_methods × 10 warmups × 6 SMC methods × 14 models
...
**Rationale for hard-R cells**: per P5.12 kickoff — VI collapses multimodal (gmm_25), funnel (neals_funnel), heavy-tail (horseshoe, stoch_vol), curved manifold (banana) posteriors to a poor Gaussian surrogate. The IMM produced is misleading, often making the sampler WORSE than no-warmup by imposing a geometry-mismatched metric.

---

#### Subgroup E: `fullrank_vi` warmup — compatible: nuts, hmc, mala, rwm, barker

Dense IMM from full-rank VI. Only viable for d≤30 (O(d²) dense matrix). P5.12: recommended for logistic_synthetic (d≈26) and lotka_volterra — both low-d smooth posteriors.
...
- stoch_vol G: d=503, AR(1) tridiagonal — this is where MCLMC shines. Continuous-momentum + diagonal IMM from tuning = favorable for tridiagonal precision. Confirmed by P4.9 outcome ("NUTS/HMC default_works=False at 503-D; MCLMC discrimination claim validated").
...
From P5.12 kickoff locked decisions. Using VI as the sampler (not as warmup) — outputs approximate posterior samples, not MCMC samples. Use only when benchmark Q is VI quality vs MCMC, or when full MCMC budget is infeasible.
...
**P5.12 applicability**: meanfield = 8 models applicable; fullrank = mvn_10 + logistic_syn + eight_schools + german_credit + lotka_volterra (d≤30 smooth); both R for the 4 hard-excluded models.
...
**Additional note**: P5.11 flagged that `adaptive_persistent_sampling` default `target_ess=3.0` causes delta=0 for N=200 particles (bisection at zero). Recipe builders must set `target_ess = 0.5 * num_particles` (not the upstream default). This is a recipe-parameter configuration issue, not a supersession issue.
...
### Category 3 — VI on pathological posteriors (from P5.12 kickoff)
