...
"""Tests for the laplace_dmhmc base method registry entry.
...
``laplace_dmhmc`` is ``laplace_dynamic_hmc`` with multinomial trajectory
sampling (``blackjax.mcmc.hmc.multinomial_hmc_proposal``).  The "2×2"
variant: dynamic step count + multinomial proposal.  The HP surface has only
``step_size`` (same as ``laplace_dhmc``).

Note: init signature is ``.init(phi_init, rng_key)`` (rng_key for step-count seeding).
...
from tuningfork.base_method.laplace_dmhmc import ENTRY
...
class TestLaplaceDMHMCEntryFields:
    """ENTRY field validation for laplace_dmhmc."""

    def test_name(self) -> None:
        assert ENTRY.name == "laplace_dmhmc"
    ...
    def test_default_hp_space_has_step_size_only(self) -> None:
        """Dynamic variant: only step_size is BO-tunable."""
        hp_names = {hp.name for hp in ENTRY.default_hp_space}
        assert "step_size" in hp_names
        assert (
            "num_integration_steps" not in hp_names
        ), "laplace_dmhmc is dynamic: num_integration_steps should NOT be in HP space"
...
class TestLaplaceDMHMCFactorySmoke:
    ...
    def test_init_requires_rng_key_returns_state_with_theta_star(self) -> None:
        """laplace_dmhmc.init takes (phi_init, rng_key) — dynamic step-count seeding."""
        algo = ENTRY.factory(
            None,
            log_joint_fn=_log_joint,
            theta_init=_THETA_INIT,
...
class TestLaplaceDMHMCEndToEnd:
...
class TestLaplaceDMHMCNoWarmup:
    ...
    def test_no_warmup_raises_not_implemented(self) -> None:
        from tuningfork.warmup.no_warmup import _runner
