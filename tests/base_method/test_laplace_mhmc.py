...
"""Tests for the laplace_mhmc base method registry entry.
...
``laplace_mhmc`` is ``laplace_hmc`` with multinomial trajectory sampling
(``blackjax.mcmc.hmc.multinomial_hmc_proposal``).  The HP surface is
...
from tuningfork.base_method.laplace_mhmc import ENTRY
...
class TestLaplaceMHMCEntryFields:
    """ENTRY field validation for laplace_mhmc."""

    def test_name(self) -> None:
        assert ENTRY.name == "laplace_mhmc"
    ...
    def test_default_hp_space_has_num_integration_steps(self) -> None:
        hp_names = {hp.name for hp in ENTRY.default_hp_space}
...
class TestLaplaceMHMCFactorySmoke:
    ...
    def test_init_returns_state_with_theta_star(self) -> None:
        algo = ENTRY.factory(
            None,
            log_joint_fn=_log_joint,
            theta_init=_THETA_INIT,
        ...
        state = algo.init(_PHI_INIT)
        assert hasattr(state, "position"), "State must have 'position' field"
        assert hasattr(
            state, "theta_star"
        ), "LaplaceHMCState must have 'theta_star' field"
        assert jnp.isfinite(
            state.logdensity
...
class TestLaplaceMHMCEndToEnd:
    ...
    def test_50_step_scan_no_nan(self) -> None:
        ...
        keys = jax.random.split(jax.random.key(_SEED), 50)
