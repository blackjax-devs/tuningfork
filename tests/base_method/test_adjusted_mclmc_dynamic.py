...
class TestAdjustedMclmcDynamicVmapSafety:
    ...
    def test_factory_callable_under_jit(self) -> None:
        ...
        @jax.jit
        def build_and_step(step_size, L, init_key, step_key):
            algo = ENTRY.factory(
                logdensity_fn,
                step_size=step_size,
                L=L,
            ...
            state = algo.init(init_pos, rng_key=init_key)
            return algo.step(step_key, state)

        # Should not raise ConcretizationTypeError
        k1, k2 = jax.random.split(key)
        new_state, info = build_and_step(jnp.array(0.1), jnp.array(1.0), k1, k2)
        assert hasattr(new_state, "position")
    ...
    def test_factory_callable_under_vmap(self) -> None:
        ...
        step_keys = jax.random.split(jax.random.fold_in(key, 1), num_chains)

        def _init_one(init_key):
            return ENTRY.factory(
                logdensity_fn,
                step_size=0.1,
                L=1.0,
                inverse_mass_matrix=jnp.ones(_D),
            ).init(init_pos, rng_key=init_key)

        init_states = jax.vmap(_init_one)(init_keys)

        def _step_one_chain(state, step_key, step_size, L):
            ...
            return kernel_step(step_key, state)

        # Should not raise ConcretizationTypeError
        new_states, infos = jax.vmap(_step_one_chain)(
            init_states, step_keys, per_chain_ss, per_chain_L
        )
    ...
    def test_avg_minimum_is_1(self) -> None:
        """When L < step_size, avg should be clamped to 1.0 (not < 1)."""
        key = jax.random.key(202)
        init_pos, logdensity_fn = _build_logdensity(_MVN, key)
        # L=0.001, step_size=1.0 → raw ratio=0.001 → clamped to 1.0
        algo = ENTRY.factory(
            logdensity_fn,
