...
from bjx_bench.inference.base_method.fullrank_vi import ENTRY, FRVISamplerState
...
class TestFRVIEntryFields:
    """ENTRY field validation for fullrank_vi."""
    ...
    def test_grad_count_returns_one(self) -> None:
        ...
        fake_info = FRVIInfo(elbo=jnp.asarray(-3.0))
...
# ---------------------------------------------------------------------------
# 3. init() returns FRVISamplerState with correct shape
# ---------------------------------------------------------------------------
...
class TestFRVIInit:
    ...
    def test_init_returns_frvi_sampler_state(self) -> None:
        algo = ENTRY.factory(_logdensity_fn, num_optimization_steps=50)
        init_pos = jnp.zeros(_D)
        state = algo.init(init_pos)
        assert isinstance(
            state, FRVISamplerState
        ), f"Expected FRVISamplerState, got {type(state)}"
    ...
    def test_init_vi_state_has_mu_chol_params(self) -> None:
        algo = ENTRY.factory(_logdensity_fn, num_optimization_steps=50)
        state = algo.init(jnp.zeros(_D))
        assert hasattr(state.vi_state, "mu"), "vi_state must have 'mu'"
        assert hasattr(
            state.vi_state, "chol_params"
        ), "vi_state must have 'chol_params'"
        assert hasattr(state.vi_state, "opt_state"), "vi_state must have 'opt_state'"

    def test_init_chol_params_shape(self) -> None:
        """chol_params must have shape (d*(d+1)//2,) for full-rank VI."""
        algo = ENTRY.factory(_logdensity_fn, num_optimization_steps=50)
        state = algo.init(jnp.zeros(_D))
        expected_len = _D * (_D + 1) // 2
        assert state.vi_state.chol_params.shape == (expected_len,), (
            f"Expected chol_params shape ({expected_len},), "
            f"got {state.vi_state.chol_params.shape}"
        )
...
class TestFRVIStep:
    ...
    def test_step_new_state_is_frvi_sampler_state(self) -> None:
        algo = ENTRY.factory(_logdensity_fn, num_optimization_steps=50)
        state = algo.init(jnp.zeros(_D))
        new_state, _info = algo.step(jax.random.key(_SEED), state)
        assert isinstance(new_state, FRVISamplerState)
    ...
    def test_step_vi_state_unchanged(self) -> None:
        ...
        state = algo.init(jnp.zeros(_D))
        new_state, _info = algo.step(jax.random.key(_SEED), state)
        # chol_params should be identical (frozen)
        assert jnp.allclose(
            new_state.vi_state.chol_params,
            state.vi_state.chol_params,
        ), "vi_state.chol_params should be unchanged after step"

    def test_step_info_has_elbo_field(self) -> None:
        algo = ENTRY.factory(_logdensity_fn, num_optimization_steps=50)
        state = algo.init(jnp.zeros(_D))
        _new_state, info = algo.step(jax.random.key(_SEED), state)
        assert hasattr(info, "elbo"), "FRVIInfo must have 'elbo' field"
...
class TestFRVIEndToEnd:
    ...
    def test_elbo_converges_on_5d_std_normal(self) -> None:
        """Final ELBO > -0.5*5*(1+log(2π)) - 1.0 after 5_000 steps."""
        from blackjax.vi.fullrank_vi import init, step
