...
"""Tests for the P5.13 dmhmc base method registry entry.

Covers:
  1. dmhmc entry exists in BASE_METHODS registry.
  2. dmhmc factory is callable and returns a SamplingAlgorithm.
  3. dmhmc kernel step advances position (state is mutated by one step).
  4. dmhmc grad_count_per_step reads info.num_integration_steps.
  5. dmhmc default_hp_space has exactly 1 BO-tunable HP (step_size);
...
# ---------------------------------------------------------------------------
# 1. Registry: dmhmc entry exists
# ---------------------------------------------------------------------------


class TestDmhmcRegistry:
    """dmhmc entry exists in BASE_METHODS and is correctly named."""

    def test_dmhmc_entry_in_registry(self) -> None:
        assert (
            "dmhmc" in BASE_METHODS
        ), f"'dmhmc' not found in BASE_METHODS; registered: {sorted(BASE_METHODS)}"

    def test_dmhmc_name_field_matches_key(self) -> None:
        entry = BASE_METHODS["dmhmc"]
        assert entry.name == "dmhmc", f"Expected name='dmhmc', got {entry.name!r}"

    def test_dmhmc_family_is_mcmc(self) -> None:
        entry = BASE_METHODS["dmhmc"]
        assert entry.family == "mcmc", f"Expected family='mcmc', got {entry.family!r}"

    def test_dmhmc_needs_mass_matrix_true(self) -> None:
        """inverse_mass_matrix comes from CHEES warmup, not BO."""
        entry = BASE_METHODS["dmhmc"]
        assert (
            entry.needs_mass_matrix is True
        ), "dmhmc.needs_mass_matrix must be True (inverse_mass_matrix from CHEES warmup)"

    def test_dmhmc_target_acceptance_rate_is_chees_default(self) -> None:
...
# ---------------------------------------------------------------------------
# 2. Factory: dmhmc factory returns a SamplingAlgorithm
# ---------------------------------------------------------------------------


class TestDmhmcFactory:
    """dmhmc factory produces a valid BlackJAX SamplingAlgorithm."""

    def test_dmhmc_factory_callable(self) -> None:
        entry = BASE_METHODS["dmhmc"]
        assert callable(entry.factory), "dmhmc.factory must be callable"

    def test_dmhmc_factory_returns_sampling_algorithm(self) -> None:
    ...
    def test_dmhmc_factory_init_returns_state_with_position(self) -> None:
        ...
        # dmhmc.init requires rng_key for random_generator_arg (same as dynamic_hmc)
...
class TestDmhmcKernelStep:
    """dmhmc kernel step produces a new state (position changes on typical step)."""

    def test_dmhmc_kernel_step_advances_position(self) -> None:
        ...
        assert not jnp.allclose(
            ...
            atol=1e-8,
        ), "Position did not change after one dmhmc step (possible kernel bug)"

    def test_dmhmc_kernel_step_returns_info_with_acceptance_rate(self) -> None:
        """dmhmc step info must include acceptance_rate (HMCInfo)."""
        ...
        ), f"dmhmc info must have acceptance_rate; got fields: {getattr(info, '_fields', dir(info))}"
        ar = float(info.acceptance_rate)
        assert 0.0 <= ar <= 1.0, f"acceptance_rate={ar} out of [0, 1]"

    def test_dmhmc_kernel_step_info_has_num_integration_steps(self) -> None:
        """dmhmc step info must include num_integration_steps (random each step)."""
        ...
        ), f"dmhmc info must have num_integration_steps; got: {getattr(info, '_fields', dir(info))}"
...
class TestDmhmcGradCount:
    """dmhmc grad_count_per_step reads info.num_integration_steps (dynamic)."""

    def test_dmhmc_grad_count_uses_num_integration_steps(self) -> None:
    ...
    def test_dmhmc_grad_count_returns_array(self) -> None:
    ...
    def test_dmhmc_grad_count_positive(self) -> None:
...
# ---------------------------------------------------------------------------
# 5. HP space: only step_size; inverse_mass_matrix and num_integration_steps absent
# ---------------------------------------------------------------------------


class TestDmhmcHpSpace:
    """dmhmc default_hp_space has exactly 1 BO-tunable HP (step_size)."""

    def _hp_names(self) -> set[str]:
        return {hp.name for hp in BASE_METHODS["dmhmc"].default_hp_space}

    def test_dmhmc_hp_space_has_only_step_size(self) -> None:
    ...
    def test_dmhmc_hp_space_no_inverse_mass_matrix(self) -> None:
        """inverse_mass_matrix must NOT be in HP space (warmup-derived from CHEES)."""
        assert (
            "inverse_mass_matrix" not in self._hp_names()
        ), "inverse_mass_matrix must NOT be in dmhmc hp_space (it's CHEES warmup-derived)"

    def test_dmhmc_hp_space_no_num_integration_steps(self) -> None:
        """num_integration_steps is NOT BO-tunable (dynamic; adapted by CHEES)."""
        assert (
            "num_integration_steps" not in self._hp_names()
        ), "num_integration_steps must NOT be in dmhmc hp_space (adapted by CHEES)"

    def test_dmhmc_step_size_is_loguniform(self) -> None:
        for hp in BASE_METHODS["dmhmc"].default_hp_space:
