"""ExpI Stage 2 Probe L3: laplace_mhmc, L=3, 80 samples/chain x 4 chains.

TL brief (2026-05-25): L=3 follow-up probe. Physical motivation: 3 x step_size=0.526
= 1.578 ≈ π/2 (half-period of whitened harmonic oscillator → theoretical optimum for
Gaussian-in-whitened-coords geometry). Budget: ~1.16 s/step x 80 + 9s ≈ 102s < 2 min.

TL robustness note: L=3 sits at the sharp π/2 optimum (fragile to step_size error).
L=5 (trajectory ≈2.63, past π/2) is on a flatter, more forgiving part of the ESS-vs-
trajectory curve. Under generic warmup (Stage 3), step_size is estimated not GT, so
robustness-vs-optimality tension matters. Read L=3 result with that in mind.

Stage 2 reference (GT params, all L same step_size=0.526, dense 3x3 IMM, maxiter=500):
  L= 5: ESS/s=1.425  vs-L10=1.110x  wall=138.3s  min_ESS=197.06
  L=10: ESS/s=1.284  vs-L10=1.000x  wall=145.3s  min_ESS=186.50  (Stage 1 ref)
  L=20: ESS/s=0.629  vs-L10=0.490x  wall=163.3s  min_ESS=102.59
"""
...
N_LEAPFROG = 3  # L=3 probe: 3 x 0.526 = 1.578 ≈ π/2
STEP_SIZE = 0.526
IMM_3X3 = jnp.array(
    [
...
# Stage 2 L=5 reference (better reference for L=3 comparison)
STAGE2_L5_ESS_PER_S = 197.06 / 138.294  # 1.425

print(
    f"  {N_CHAINS} chains x {N_SAMPLES} samples/chain = {N_CHAINS * N_SAMPLES} total, "
    f"step_size={STEP_SIZE}, L={N_LEAPFROG} (π/2 probe), maxiter={MAXITER}",
    flush=True,
)
print(
    f"  Stage 1 L=10 reference: ESS/s={STAGE1_ESS_PER_S:.4f}  "
    f"Stage 2 L=5 reference: ESS/s={STAGE2_L5_ESS_PER_S:.4f}",
    flush=True,
)
...
ratio_vs_l5 = ess_per_sec / STAGE2_L5_ESS_PER_S
...
print(f"\n{sep}", flush=True)
print(
    f"ExpI Stage 2 L=3 Probe Summary (total wall = {t_total - t0:.1f}s)",
    flush=True,
)
print(
...
print(
    ...
    f"vs-L10={ratio_vs_l10:.3f}x  vs-L5={ratio_vs_l5:.3f}x",
    flush=True,
)
print(f"  max_R-hat={max_rhat:.4f}  (pass if < 1.05)", flush=True)
print(
    f"  Stage 1 L=10: ESS/s={STAGE1_ESS_PER_S:.5f}  "
    f"Stage 2 L=5: ESS/s={STAGE2_L5_ESS_PER_S:.5f}",
    flush=True,
)
print(sep, flush=True)
