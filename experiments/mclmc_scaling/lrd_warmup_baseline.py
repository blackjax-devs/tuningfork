# VERBATIM COPY of blackjax/blackjax/adaptation/mclmc_lrd_adaptation.py
# (as of blackjax commit 359205da, 2026-06-10).
#
# PURPOSE: OLD baseline for the A/B comparison in sweep_warmup_compare.py.
# DO NOT MODIFY — must remain behaviorally identical to the shipped warmup.
# Any behaviour change must go into lrd_warmup_enhanced.py.
#
...
def mclmc_lrd_warmup(
    ...
    *,
    ...
    """Scheme A (pilot-free) MCLMC warmup — BASELINE (shipped behaviour).

    This is a verbatim copy of the shipped ``blackjax.mclmc_lrd_warmup``
    (``blackjax.adaptation.mclmc_lrd_adaptation``).  It is the OLD baseline for
    the A/B comparison in ``sweep_warmup_compare.py``.

    See ``lrd_warmup_enhanced.py`` for the enhanced version.
    """
    ...
    # OLD BASELINE: k_safe = floor(n_eff/2) — the shipped conservative rank guard.
    k_safe = int(n_eff / 2)
    ...
    # OLD BASELINE: warm-start from the pilot's own adapted (L, step_size).
    # The pilot used diagonal preconditioning; in LRD geometry the step optimum
    # can be ~5x higher — the DA tuner has to climb from below.
