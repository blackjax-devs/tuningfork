...
# ---- PARITY GATE on banana itself (avg=18, n=300) ----
print("PARITY CHECK: vmap vs sequential loop (banana, avg=18, n=300, 4 chains)...")
_ld, _init, _d, _imm = load_banana()
_rs0 = ref_step(_ld, _init, _imm, SEEDS[0])
_a_loop, _dl, _al, _nl = sample_loop(_ld, _init, _imm, _rs0, 18, 300, CH, SEEDS[0])
_a_vmap, _dv, _av, _nv = sample_vmap(_ld, _init, _imm, _rs0, 18, 300, CH, SEEDS[0])
...
sample = sample_vmap if USE_VMAP else sample_loop
print(
    "  PARITY OK -> using vmap (fast path)"
    if USE_VMAP
    else f"  !! PARITY FAILED (max|d|={_max_abs:.3e}) -> FALLING BACK to sequential loop"
)
sys.stdout.flush()
...
out = os.path.join(_HERE, "cert_banana_medium18_results.json")
...
print(
    f"wrote {out} ({len(cells)} rows) | path={'VMAP' if USE_VMAP else 'LOOP'} | parity={_max_abs:.3e}"
)
print(f"RECOMMENDED_FILING_AVG={rec}")
print("DONE_CERT_BANANA18")
