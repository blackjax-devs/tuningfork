"""ExpI Stage 2 Probe 2b: laplace_mhmc, L=20, 30 samples/chain x 4 chains.

TL brief (2026-05-25): L=20 arm of the optimal-L calibration for laplace_mhmc.
Stage 1 winner: laplace_mhmc (fixed-L=10, multinomial) — ESS/s=1.284.
Question: does doubling the trajectory (L=20) improve ESS/sample enough to
  outweigh the per-step cost increase?
  Verdict rule: act only on CLEAR L20>1.1×L10 signal; treat marginal
  differences as noise (n=30 carries ~40% rel. uncertainty on ESS/s).

Budget: ~3.0-3.5 s/step (L=20; more leapfrog) x 30 + 9s ≈ 99-114s < 2 min.
...
MAXITER = 500
N_CHAINS = 4
N_SAMPLES = 30  # per chain; 120 total — kept small (L=20 less likely winner)
N_LEAPFROG = 20  # Probe 2b: L=20 (double of Stage 1 L=10)
STEP_SIZE = 0.526
IMM_3X3 = jnp.array(
    [
...
print(
    ...
    f"step_size={STEP_SIZE}, L={N_LEAPFROG} (Probe 2b), maxiter={MAXITER}",
    flush=True,
)
print(
...
print(
    "  Verdict: act only on CLEAR L20 > 1.1x L10 = "
    f"{1.1 * STAGE1_ESS_PER_S:.4f} ESS/s",
    flush=True,
)
...
print(f"\n{sep}", flush=True)
print(
    f"ExpI Stage 2 Probe 2b Summary (total wall = {t_total - t0:.1f}s)",
    flush=True,
)
print(
    "  blackjax: stock main 007a9ded  |  "
    "gp_regression N=200  |  V2 vmap  |  4 chains x 30 samples",
    flush=True,
)
...
print(
    ...
    f"vs-L10={ratio_vs_l10:.3f}x  (verdict: act only if >1.10)",
    flush=True,
)
