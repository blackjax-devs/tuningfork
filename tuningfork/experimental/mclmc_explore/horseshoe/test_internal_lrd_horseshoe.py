...
"""Test upstream LRD-dispatching isokinetic_mclachlan with adjusted_mclmc_dynamic on horseshoe (204-D).

The custom lrd_integrator.py has been retired.  We now use the standard
blackjax.mcmc.integrators.isokinetic_mclachlan, which dispatches to the LRD
path when passed a LowRankInverseMassMatrix (blackjax PR #936).
"""
...
def run_internal_lrd_mclmc_dynamic(
    ...
    *,
    ...
    """Run adjusted_mclmc_dynamic using the upstream LRD-dispatching isokinetic_mclachlan on horseshoe."""
    ...
    # 1. Warmup / Adaptation (Unadjusted LRD kernel to adapt L and step_size)
    @jax.vmap
    ...
    # Print adapted parameters for debugging
    print(f"Adapted step_size: {adaptation_states.step_size}")
    print(f"Adapted L: {adaptation_states.L}")

    # 2. Parallel Sampling with custom dynamic adjusted LRD kernel
    @jax.vmap
    def run_sampling_one(k, state_pos, params):
        init_k, sample_k = jax.random.split(k)

        # Initialize DynamicHMCState
        state = blackjax.mcmc.adjusted_mclmc_dynamic.init(
            state_pos, logdensity_fn, init_k
        )

        # Construct custom dynamic adjusted kernel using the upstream isokinetic_mclachlan,
        # which now dispatches natively on LowRankInverseMassMatrix (blackjax PR #936).
        steps_fn = (
        ...
        # Scale down step_size to ensure high acceptance probability in adjusted sampler
        step_size_adj = params.step_size * 0.55
        avg = jnp.maximum(1.0, params.L / step_size_adj)

        def body_fn(state, rng_key):
            # Pass our LowRankInverseMassMatrix directly into the custom step function!
        ...
        _, (positions, infos) = jax.lax.scan(
            body_fn,
            state,
            jax.random.split(sample_k, n_samples),
        )
        return positions, infos

    # Squeeze the position from adapted_states
...
def main():
    ...
    # --- Step 3: Run Internal LRD adjusted_mclmc_dynamic ---
    print(
        f"\n[MCLMC Execution] Running Custom Internal LRD adjusted_mclmc_dynamic (k={k}) on horseshoe (204-D)..."
    )
    t0 = time.perf_counter()
