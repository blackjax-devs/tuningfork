...
proposal with a sigmoid accept/reject step. ``step_size`` is recipe-resolved;
...
generated plans, the scalar default is ``step_size``; the mass matrix is either
left as ``None`` (identity) or supplied by warmup adaptation.
...
ENTRY = BaseMethod(
    ...
    notes=(
        ...
        "(defaults to identity/None if no warmup). Generated plans resolve "
        "step size and pass IMM separately."
    ),
)
