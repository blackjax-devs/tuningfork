...
WARMUPS: dict[str, Warmup] = {
    ...
    "staged_adaptation_auto": Warmup(
        ...
        notes=(
            ...
            "controller's growing-window schedule and rank-detection floor. It "
            "also sizes the metric-fitting ring buffer, jointly with n_chains: "
            "upstream derives a per-chain buffer of B draws from "
            "max_grad_budget and n_chains (B=166 for max_grad_budget=20000, "
            "n_chains=6, d=10 on blackjax 1.6.3.dev25+g9e128d206). A slow "
            "window longer than B overwrites its oldest draws and the "
            "candidate metric is fitted on the most recent B, with nothing in "
            "the public info stream reporting the loss -- so treat "
            "max_grad_budget as a capacity choice, not only a cost cap, and do "
            "not infer consumed support from window length. "
