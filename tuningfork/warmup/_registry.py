...
WARMUPS: dict[str, Warmup] = {
    ...
    "staged_adaptation_auto": Warmup(
        ...
        notes=(
            ...
            "blackjax.staged_adaptation(metric='auto') controller: one call pools "
            "the W warmup chains, updates on mean acceptance, and publishes one "
            "shared step_size and LowRankInverseMassMatrix (sigma, U, lam); "
            "lam=1 is the diagonal rank-zero outcome, not an error. "
            "warmup_num_chains=[W] selects W in {1, num_chains}; this is joint "
            "adaptation, unlike the independent window-adaptation W=S route. "
            "The payload uses geometry_scope='shared' and the existing "
            "'low_rank_inverse_mass_matrix' marker; no schema change. "
            "max_grad_budget is required under metric='auto' and is both a "
            "growing-window/per-chain-buffer capacity choice and a budget; do not "
            "infer consumed support from window length. n_warmup is passed to "
            "run() explicitly. Compatible methods are nuts, hmc and mhmc because "
            "they expose num_integration_steps; barker lacks that cost field, "
            "mala/ghmc/dynamic_hmc fail the upstream kernel contract, and rmhmc is "
            "excluded by upstream documentation. W>1 requires a BlackJAX accepting "
            "n_chains and fails explicitly otherwise; W=1 remains portable. "
            "Upstream warns below W=6; prior_sample gives all chains the same start "
            "unless a per-chain init_strategy disperses them. A short growing-window "
            "run may produce a runaway step_size without warning, so inspect it. "
            "These cautions are observations at particular pins, not constants."
        ),
    ),
