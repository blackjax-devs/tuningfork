...
def _build_stationary_init_positions(
    ...
    """Build deterministic per-chain positions from a reference summary."""
    summary_path = catalog_root / model_name / "reference" / "summary.json"
    if not summary_path.exists():
        raise FileNotFoundError(
            f"Reference summary not found at {summary_path} for model {model_name!r}."
        )
    ...
    positions = [
        {
            name: jnp.asarray(mean, dtype=dtype)
            + offsets[chain % len(offsets)] * jnp.asarray(stds[name], dtype=dtype)
            for name, mean in means.items()
        }
        for chain in range(num_chains)
    ]
    return jax.tree.map(lambda *values: jnp.stack(values), *positions)
...
def emit_low_recipe_for_cell(
    ...
    *,
    ...
    if is_elliptical_slice:
        ...
        # rhat>>1 after 1000 steps. The reference summary places chains at
        # mean ± small offsets,
        ...
        if init_strategy is None:
            try:
                ...
                # The helper returns (1, ...) batched; squeeze.
    ...
    recipe_kwargs: dict[str, Any] = dict(
        ...
        # re-run time via the generated execution path.
