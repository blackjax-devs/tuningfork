...
historical corpus-wide config-fidelity gate
(`tools/verify_emitted_configs.py` at `d702b7e`, wired into CI by PR #256 four
seconds before #257 merged, so #257 never saw it)
...
shape as the 16 pre-existing entries in the historical
`PRECISION_FLIP_CELLS` (`tools/reemit_sweep.py` at `d702b7e`); the re-emit is
arguably the more-correct run.
