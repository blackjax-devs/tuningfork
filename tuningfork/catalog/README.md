...
| `recipes/{low,medium,high,failed}__<sampler>__<warmup>.json` | Per-cell recipes from recipe-generation pipeline (R5 ships 7 canonical FAILED recipes; LOW/MEDIUM/HIGH land as recipe sweeps execute) | yes |
...
- **`recipes/` subdirectories** are populated by the recipe-generation pipeline. **LOW** recipes shipped during 2026-05 (45 LOW recipes across 12 models). **MEDIUM** recipes are shipping during 2026-05. **7 canonical FAILED** recipes from R5 (2026-05-17) document hard-exclusion categories — see [`gmm_25/recipes/failed__nuts__window_adaptation_diag_imm.json`](gmm_25/recipes/failed__nuts__window_adaptation_diag_imm.json) for the multimodal × single-chain-gradient exclusion example. HIGH recipes are not yet shipped.
