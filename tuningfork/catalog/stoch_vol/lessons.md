...
**⚠️ PRNG fragility (2026-05-17 discovery)**: the canonical config above passes cleanly at `jax.random.key(0)` (R̂=1.0006, ESS=1992, 105 div, wall ≈ 3 min) but catastrophically fails at `jax.random.key(20260517)` (R̂≈5, ESS≈0.5, 0 div, wall ≈ 10 min — chain stuck, never mixes). The "quiet failure" mode (`n_div=0` + non-mixing) is caught by the split-R̂ + min-bulk-ESS gate, NOT by divergence count. Because of this fragility, **the shipped `groundtruth_samples/blackjax/draws.npz` for stoch_vol is the Phase 0 `seed=0` trace** (not the 2026-05-17 sweep seed used by the other 12 catalog models). The catalog README's "per-model seed policy" section documents the two-seed pinning. Multi-seed protocol robustness is an open investigation — see [`worklog/lessons/case-studies/stoch_vol/2026-05-17-prng-fragility-recert.md`](worklog/lessons/case-studies/stoch_vol/2026-05-17-prng-fragility-recert.md) for the full diagnostic write-up and reproduction recipe.
